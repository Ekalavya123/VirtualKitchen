 ...
 */
export default function RecipeProcessListPage({ recipeId, currentUserId, onOpenProcess, onBack }: RecipeProcessListPageProps) {
  ...
  return (
    <div className="flex h-full w-full flex-col" style={{ background: 'var(--flow-surface-muted)' }}>
      ...
      <div className="flex-1 overflow-y-auto p-5" style={{ maxWidth: 760, margin: '0 auto', width: '100%' }}>
        ...
        {mainProcess ? (
          <RecipeProcessSummaryCard process={mainProcess} onOpen={() => onOpenProcess(mainProcess.id)} />
        ) : (
          <div
        ...
        {subprocesses.length === 0 ? (
          ...
          <div className="flex flex-col gap-3">
            {subprocesses.map((process) => (
              <RecipeProcessSummaryCard key={process.id} process={process} onOpen={() => onOpenProcess(process.id)} />
            ))}
          </div>
        )}
      </div>
