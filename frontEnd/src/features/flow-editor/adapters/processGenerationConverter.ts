/**
 * Converts an AI-generated semantic Process structure (recipe/dto/Generated*.java via
 * ProcessGenerationApi) into the same domain `Process[]` shape RecipeSessionContext already works
 * with — generating every React Flow presentation detail (node/edge ids, positions, dimensions)
 * the AI was deliberately never asked for, mirroring how RecipeExecutionGraphConverter.ts does the
 * equivalent job for the legacy flow model's AI generation. Subprocess `ref` strings are resolved
...
import { normalizeProcessStepNodeData, type ProcessStepNodeData } from '../model/processStepData'
import { parseDurationLabel } from '../catalog/stepFieldCatalog'
import type {
  GeneratedProcess,
  GeneratedProcessStep,
  Process,
  ProcessEdge,
...
const buildStepNode = (step: GeneratedProcessStep, refToProcessId: Map<string, number>, x: number, y: number): ProcessNode => {
  const data: ProcessStepNodeData = normalizeProcessStepNodeData({
    step: {
      action: step.action ?? '',
      customActionName: '',
      actionOn: {
...
const buildConditionNode = (step: GeneratedProcessStep, x: number, y: number): ProcessNode => {
 ...
 * order (the AI was asked for an ordered list, not a graph — see ProcessGenerationPromptBuilder),
 ...
 */
const buildNodesAndEdges = (steps: GeneratedProcessStep[], refToProcessId: Map<string, number>): { nodes: ProcessNode[]; edges: ProcessEdge[] } => {
...
export const convertGeneratedResultToProcesses = (
  recipeId: number,
  result: ProcessGenerationResult,
  ...
  const buildProcess = (generated: GeneratedProcess, id: number, type: Process['type']): Process => {
