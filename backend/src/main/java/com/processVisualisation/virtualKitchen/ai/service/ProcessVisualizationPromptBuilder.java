...
/**
 * Builds AI prompts for Process-model STEP visualization (image only — no video in V1). Distinct
 * from {@link AIVisualizationPromptBuilder}, which reads the legacy flat flow's field vocabulary;
 * this reads {@link ProcessVisualizationInput}, the Process model's own Action On/ingredients/
 * subprocess-reference shape.
 * <p>
 ...
 */
@Component
public class ProcessVisualizationPromptBuilder {
    ...
    public String buildUserPrompt(ProcessVisualizationInput current) {
    ...
    private String describeStep(ProcessVisualizationInput step) {
        ...
        if (!step.ingredients().isEmpty()) {
            ...
            for (int i = 0; i < step.ingredients().size(); i++) {
                ProcessVisualizationInput.IngredientTarget ingredient = step.ingredients().get(i);
    ...
    private List<String> advancedProperties(ProcessVisualizationInput step) {
