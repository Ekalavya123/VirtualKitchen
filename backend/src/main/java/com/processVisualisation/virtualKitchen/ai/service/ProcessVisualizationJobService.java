...
/**
 * Orchestrates async, per-step Process-model visualization generation — the same
 * {@code TaskPool}-based pipeline {@link VisualizationJobService} uses for the legacy flow model
 * (same bounded "visualization" worker pool, same "orchestrator" pool, same {@link VisualizationJob}
 * collection — {@code processId} distinguishes a Process-model job from a legacy one), scoped to one
 * process's own STEP nodes rather than a whole recipe flow.
 */
@Service
public class ProcessVisualizationJobService {

    private static final Logger log = LoggerFactory.getLogger(ProcessVisualizationJobService.class);
    ...
    public ProcessVisualizationJobService(
            ProcessVisualizationService processVisualizationService,
        ...
        this.processVisualizationService = processVisualizationService;
    ...
    public VisualizationJobResponseDTO startJob(Long userId, Long recipeId, Long processId) {
        ProcessVisualizationService.ProcessStepPreparation preparation =
                processVisualizationService.prepareStepContexts(processId);
    ...
    public VisualizationJobResponseDTO getJobStatus(String jobId) {
        VisualizationJob job = visualizationJobRepository.findById(jobId)
                .orElseThrow(() -> new RecipeFlowGenerationException("Process visualization job not found: " + jobId));
        return toDto(job);
    }

    private void runPipeline(Long userId, Long recipeId, String jobId, ProcessVisualizationService.ProcessStepPreparation preparation) {
        ...
        try {
            List<NamedTask<VisualizationAsset>> tasks = preparation.steps().stream()
                    .map(ctx -> new NamedTask<VisualizationAsset>(
                            ctx.node().getId(),
                            () -> processVisualizationService.resolveVisualizationAsset(
            ...
            processVisualizationService.attachResultsAndSave(preparation.process(), assetsByStepId);
