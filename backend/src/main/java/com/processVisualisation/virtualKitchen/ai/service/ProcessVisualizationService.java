 ...
 * nodes are never visualized). Structurally mirrors {@link AIRecipeVisualizationService} — same
 * {@link VisualizationAsset} lookup-fill-or-create lifecycle, same durable-artifact-before-upload
 * sequencing via {@link AiArtifactService}/{@link VisualizationImageArtifactConsumer} — but reads
 * the Process model's own Action On (ingredients + subprocess references) shape instead of the
 * legacy flat flow's field vocabulary, and is scoped to one {@link Process} (a MAIN or a
...
public class ProcessVisualizationService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ProcessVisualizationService.class);
    ...
    private final ProcessVisualizationPromptBuilder promptBuilder;
    ...
    private final StepVocabularyProvider vocabularyProvider;
    private final ObjectMapper objectMapper = new ObjectMapper();

    public ProcessVisualizationService(
            ...
            ProcessVisualizationPromptBuilder promptBuilder,
            ...
            StepVocabularyProvider vocabularyProvider
    ) {
     ...
     * @throws RecipeFlowGenerationException if the process cannot be found
     */
    public ProcessStepPreparation prepareStepContexts(Long processId) {
        Process process = processRepository.findById(processId)
                .orElseThrow(() -> new RecipeFlowGenerationException("Process not found: " + processId));
        ...
        for (Process.ProcessNode node : orderedSteps) {
            ...
            ProcessVisualizationInput input = extractVisualizationInput(data, subprocessNameById, previousExpectedOutput);
     ...
     */
    public VisualizationAsset resolveVisualizationAsset(Long userId, Long recipeId, String stepId, ProcessVisualizationInput input) {
    ...
    public record StepContext(Process.ProcessNode node, ProcessVisualizationInput input) {
    }
    ...
    private VisualizationAsset generatePrompt(Long userId, String visualizationKey, ProcessVisualizationInput input) {
        try {
     ...
     * storage failure cannot destroy a paid generation — identical durability sequencing to
     * {@link AIRecipeVisualizationService#generateImage}: the provider's bytes are staged as an
    ...
    private VisualizationAsset createAsset(Long userId, String visualizationKey, ProcessVisualizationInput input) {
     ...
     * continuity is accurate — identical reasoning to
     * {@link AIRecipeVisualizationService}'s own node ordering: two STEPs separated by a CONDITION
    ...
    private ProcessVisualizationInput extractVisualizationInput(
        ...
        List<ProcessVisualizationInput.IngredientTarget> ingredients = extractIngredients(step);
        ...
        return new ProcessVisualizationInput(
                action,
                ingredients,
                subprocessNames,
    ...
    @SuppressWarnings("unchecked")
    private List<ProcessVisualizationInput.IngredientTarget> extractIngredients(Map<String, Object> step) {
        ...
        List<ProcessVisualizationInput.IngredientTarget> targets = new ArrayList<>();
        for (Object entry : (List<Object>) rawIngredients) {
            ...
            targets.add(new ProcessVisualizationInput.IngredientTarget(name, quantity, unit, preparationStyle));
        }
        return targets;
    }
